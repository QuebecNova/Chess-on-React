...
export class UniversalBoardDrawer {
    constructor(boardElem, config) {
        ...
        if (!this.document) {
            if (this.debugMode)
                console.error(`Inputted document element doesn't exist!`)
        ...
        if (!this.boardElem) {
            if (this.debugMode)
                console.error(`Inputted board element doesn't exist!`)
        ...
        if (typeof this.boardDimensions != 'object') {
            if (this.debugMode)
                console.error(
                    `Invalid board dimensions value, please use array! (e.g. [8, 8])`
                )
    ...
    createArrowBetweenPositions(from, to, config) {
        const fromCoordinateObj = this.squareSvgCoordinates.find(
            (x) => this.coordinateToFen(x.coordinates) == from
        )
        const toCoordinateObj = this.squareSvgCoordinates.find(
            (x) => this.coordinateToFen(x.coordinates) == to
        )
        ...
        if (!fromCoordinateObj || !toCoordinateObj) {
            if (this.debugMode)
                console.error(
                    ...
                    from,
                    to,
                    'do not exist. Possibly out of bounds?'
                )
        ...
        const arrowElem =
            typeof existingArrowElem === 'object'
                ? existingArrowElem
        ...
        arrowElem.setAttribute(
            'transform',
            `rotate(${angle * (180 / Math.PI) - 90} ${fromX} ${fromY})`
        )
        ...
        const pointsString = arrowPoints
    ...
    createTextOnSquare(square, config) {
        ...
        if (!squareCoordinateObj) {
            if (this.debugMode)
                console.error(
                    'Coordinate',
                    square,
                    'does not exist. Possibly out of bounds?'
                )
        ...
        const textElem = this.document.createElementNS(
            'http://www.w3.org/2000/svg',
            'text'
        )
    ...
    createRectOnSquare(square, config) {
        ...
        const rectElem = this.document.createElementNS(
            'http://www.w3.org/2000/svg',
            'rect'
        )
    ...
    createDotOnSVG(x, y) {
        const dot = this.document.createElementNS(
            'http://www.w3.org/2000/svg',
            'circle'
        )
    ...
    updateShapes() {
        if (this.debugMode) {
            ...
            this.squareSvgCoordinates.forEach((x) =>
                this.createDotOnSVG(...x.positions)
            )
        }
            ...
            .forEach((shapeObj) => {
                switch (shapeObj.type) {
                    case 'text':
                        ...
                        const newTextElement = this.createTextOnSquare(
                            shapeObj.positions,
                            shapeObj.config
                        )
                    ...
                    case 'rectangle':
                        // shapeObj.positions is a string, just one square fen
                        const newRectElem = this.createRectOnSquare(
                            shapeObj.positions,
                            shapeObj.config
                        )
                    ...
                    default:
                        const newArrowElem = this.createArrowBetweenPositions(
                            ...shapeObj.positions,
                            shapeObj.config
                        )
    ...
    createShape(type, positions, config) {
        ...
        if (this.terminated) {
            if (this.debugMode)
                console.warn(
                    'Failed to create shape! Tried to create shape after termination!'
                )
        ...
        if (!this.boardContainerElem) {
            if (this.debugMode)
                console.warn(
                    `Failed to create shape! Board SVG doesn't exist yet! (createOverlaySVG() failed?)`
                )
        ...
        switch (type) {
            ...
            default:
                const arrowElement = this.createArrowBetweenPositions(
                    ...positions,
                    config
                )
        ...
        for (let y = 0; this.boardDimensions.height > y; y++) {
            for (let x = 0; this.boardDimensions.width > x; x++) {
                this.squareSvgCoordinates.push({
                    coordinates: [x + 1, y + 1],
                    positions: [
                        this.squareWidth / 2 + this.squareWidth * x,
    ...
    updateDimensions() {
        ...
        let boardPositionTop = boardRect.top - bodyRect.top,
            boardPositionLeft =
                boardRect.left - (this.ignoreBodyRectLeft ? 0 : bodyRect.left)
    ...
    createOverlaySVG() {
        ...
        let oldBoardRect = JSON.stringify(
            this.boardElem.getBoundingClientRect()
        )
        ...
        const additionalCheckLoop = setInterval(() => {
            ...
            const boardRect = JSON.stringify(
                this.boardElem.getBoundingClientRect()
            )
    ...
    getCoordinatesFromInputPosition(e) {
        ...
        return isOutOfBounds
            ? [null, null]
            : [
                  Math.floor(relativeX / this.squareWidth) + 1,
                  Math.floor(relativeY / this.squareHeight) + 1,
              ]
    }
    ...
    handleMouseEvent(e) {
        ...
        if (positionStr != this.lastInputPositionStr) {
            const enteredSquareListeners = this.customActivityListeners.filter(
                (obj) => obj.square == this.coordinateToFen(position)
            )

            enteredSquareListeners.forEach((obj) => obj.cb('enter'))

            if (this.lastInputPosition && this.lastInputPosition[0] != null) {
                const leftSquareListeners = this.customActivityListeners.filter(
                    (obj) =>
                        obj.square ==
                        this.coordinateToFen(this.lastInputPosition)
                )
    ...
    addSquareListener(square, cb) {
        ...
        return {
            remove: () => {
                this.customActivityListeners =
                    this.customActivityListeners.filter(
                        (obj) => obj.square != square && obj.cb != cb
                    )
            },
        }
    }
        ...
        for (let sum = 0; sum <= totalSteps; sum++) {
            ...
            for (let x = 0; x <= sum; x++) {
                ...
                createdElements.push(
                    BoardDrawer.createShape('rectangle', square, {
                        style: rectStyle,
                    })
                )
