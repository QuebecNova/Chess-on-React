...
export function checkForEnpassant
    (squares : keyableSquares, dropField : string, from : string, enpassantAvailable : string) : keyableSquares {
    ...
    if (piece.color === 'White' && enpassantAvailable.includes('Left')) enpassantedField = alphs.changeAlphPos(from, '-', 1)
    if (piece.color === 'White' && enpassantAvailable.includes('Right')) enpassantedField = alphs.changeAlphPos(from, '+', 1)
    if (piece.color === 'Black' && enpassantAvailable.includes('Left')) enpassantedField = alphs.changeAlphPos(from, '-', 1)
    if (piece.color === 'Black' && enpassantAvailable.includes('Right')) enpassantedField = alphs.changeAlphPos(from, '+', 1)
    if (dropField === alphs.changeAlphPos(from, '+', 1, '+', 1) 
